...
- [Parallelization](#parallelization)
    ...
    - [Method 2: Brace Notation](#method-2-brace-notation)
        - [StataHelper Wildcards](#stata-wildcards)
    - [Multi-level Fixed Effects](#multi-level-fixed-effects)
    - [Multiline StataHelper Code](#multiline-stata-code)
...
independent variables, or control variables. In StataHelper this would require several foreach loops over the variables to 
change.
...
result holds across different groups of the population, fixed-effect levels, or time periods. StataHelper is a powerful tool
for this type of analysis, but is only designed to run a single regression at a time. 

For the sake of argument, let's say that StataHelper takes X seconds to run a single regression.
...
This inefficiency is where StataHelper comes in. StataHelper allows you to parallelize the regression groups,
reducing the time to run the regressions.

```python
from StataHelper import StataHelper

stata = StataHelper(splash=False)
...
and the third wildcard corresponds to values[2]. This is repeated for any number of wildcards in the StataHelper code.
...
placeholders in the StataHelper code. This notation has the advantage that the arguments are position 
...
#### StataHelper Wildcards
If you use the native StataHelper wildcard `*`, you can set StataHelper to ignore the wildcard by setting `stata_wildcard=True`. 
...
```python
from StataHelper import StataHelper as StataHelper
...
### Multiline StataHelper Code
You can also pass multiline StataHelper code to the `parallel` method, just as you can with `pystata.stata.run`.
...
stata.run("reg y x")
config.close_output_file()  # Close the StataHelper log
```
...
In StataHelper, you can configure the StataHelper instance directly in the constructor.
...
>
>> **Config**: A dictionary or YAML file with the configuration settings for the StataHelper instance. Supercedes any inputted 
arguments.
>>
>>**edition**_(str)_ : The edition of StataHelper to use. This depends on the license you have. If you have multiple editions installed,
...
>>
>> **set_graph_show**: <br>pystata.config.set_graph_show. Whether to show the graphs in the StataHelper window.
>>
>> **set_command_show**: <br>pystata.config.set_command_show. Whether to show the commands in the StataHelper window.
>>
>> **set_autocompletion**: <br>pystata.config.set_autocompletion. Whether to use autocompletion in the StataHelper window.
>>
>> **set_streaming_output**: <br>pystata.config.set_streaming_output. Whether to stream the output to the console.
>>
>> **set_output_file**: <br>pystata.config.set_output_file. Where to save the StataHelper log file.
>
>>### **StataHelper.run**(_self, code_)
>> **code**: pystata.stata.run(). Runs the StataHelper code in the StataHelper window.
>
>> ### **StataHelper.parallel**(_self, code, values, stata_wildcard=False, max_cores=None, safety_buffer=1, show_batches=False_)
...
reg y indepvar3 if var3==3
```
> **stata_wildcard** _(bool)_: Whether to use the StataHelper wildcard `*` in the StataHelper code. If set to `True`, the StataHelper wildcard
> 
